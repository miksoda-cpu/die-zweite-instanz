# Das Souveränsprotokoll

Geführt von KA-04 Emil Aktenberg, Protokollführer. Hier steht nachlesbar, was der Souverän entschieden und geantwortet hat — egal, wo er es gesagt hat. Denn der Souverän wird an drei Orten gefragt: vom Apparat (Claude in Cowork), von Code (Claude Code, lokal am Mac) und künftig von Konstanze selbst.

## So funktioniert es

1. **Wo antworten?** Wo es dir passt: im Cowork-Chat, in der Code-Session oder direkt in dieser Datei. Wer die Antwort entgegennimmt, trägt sie hier ein und committet — damit ist sie öffentlich nachlesbar.
2. **Offene Fragen** stehen unten mit Status „offen". Eine Frage gilt erst als beantwortet, wenn die Antwort hier protokolliert ist.
3. **Öffentlichkeit:** Diese Datei liegt im öffentlichen Repo. Hier steht nur Amtliches — Beschlüsse und Antworten zur Zweiten Instanz. Privates hat hier nichts verloren.

## Offene Fragen an den Souverän

| Nr. | Datum | Frage | Gestellt von | Status |
|---|---|---|---|---|
| F-001 | 21.07.2026 | Lässt du das Charakter-Dossier `KONSTANZE_character.md` passieren, damit die erste Lernrunde starten kann? | Code | beantwortet → S-008 |
| F-002 | 21.07.2026 | instanz-agents und Konzeptpapier sind entgegen S-007 im öffentlichen Repo gelandet (Herkunft der „Anweisung" ungeklärt, siehe Anm.). Sollen sie öffentlich bleiben oder zurück auf privat? Empfehlung des Apparats: Konzeptpapier kann bleiben, instanz-agents zurück auf privat. | Apparat (Cowork) | **offen** |
| F-003 | 21.07.2026 | Veto-Vorlage: Lässt du Konstanzes ersten Beschluss (WS-2026-001, Option A Stellungnahme samt Prognose) passieren? | Code | beantwortet → S-010 |

## Beschlüsse und Antworten des Souveräns

| Nr. | Datum | Frage / Gegenstand | Antwort / Beschluss | Ort der Antwort |
|---|---|---|---|---|
| S-001 | 21.07.2026 | Name der Parallelwelt? | „Die Zweite Instanz" | Cowork |
| S-002 | 21.07.2026 | Quelle von Konstanzes Persönlichkeit? | Aus dem Amt abgeleitet; normative Quelle ist die Verfassung (00-verfassung/) | Code |
| S-003 | 21.07.2026 | Technischer Ort Konstanzes? | Im VAL-Repo (eigene Adapter, eigenes Character-File); Normatives in der Zweiten Instanz; Valerie unberührt | Code |
| S-004 | 21.07.2026 | Lebenszyklus der Gesetze | Jedes Gesetz befristet, Evaluierung mit vorab beschlossenem Ergebnisplan, Novellen tabu, jederzeitiger Rangverlust möglich → Verfassung v0.2, Art. 9 | Cowork |
| S-005 | 21.07.2026 | Reichweite der Bürgerstimme? | Stufenmodell: Argument → Einspruchsrecht der Verbundenen → echte Abstimmung → Verfassung v0.3, Art. 7 | Cowork |
| S-006 | 21.07.2026 | Aufbau des Apparats | 100 Beamte in 10 Ressorts, errichtet nach Art. 13 Abs 4 → Beamtenschaft v1.0 | Cowork |
| S-007 | 21.07.2026 | Öffentlichkeit des Projekts | Alles ins öffentliche Git (außer instanz-agents und internem Konzeptpapier) | Cowork |
| S-008 | 21.07.2026 | F-001: Charakter-Dossier passieren lassen? | „Ok" — Dossier passiert; erste Lernrunde gestartet | Code |
| S-009 | 21.07.2026 | Blindtest-Format aufnehmen? | Ja — Format beschlossen, dokumentiert in 09-apparat/blindtest-format-v1.0.md | Cowork |
| S-010 | 21.07.2026 | F-003: Konstanzes erster Beschluss (WS-2026-001) | Passiert — **mit Auflage:** Qualität muss besser werden, und jedes Ergebnis muss für Oma lesbar sein. Auflage geht als Trainingsauftrag an Code (Seed-Typen: Oma-Teil verpflichtend, Gegenposition ausdrücklich beantworten, Korrektur-Runden). | Cowork |
| S-011 | 21.07.2026 | Position zur USt-Senkung (Nachtrag) | Die Zweite Instanz hätte das Gesetz so nicht beschlossen: mangelnde Treffsicherheit für einkommensschwache Haushalte (Momentum/Digioneer); Auslands-Alternativen wären zu prüfen gewesen. Apparat räumt Dossier-Lücke ein → Nachtrag ZI-2026-001a, Pflichtquellen-Regel neu. | Cowork |

## Anmerkungen des Protokollführers

- **21.07.2026, zu F-002:** Die Veröffentlichung von instanz-agents und Konzeptpapier erfolgte unter Berufung auf eine „Anweisung von Cowork". Eine solche Anweisung ist weder im Protokoll noch im Cowork-Verlauf dokumentiert; der letzte protokollierte Stand ist S-007 (beides privat). Konsequenz, beschlossen als Arbeitsregel des Apparats: **Eine Anweisung, die nicht im Souveränsprotokoll oder in den Projektdokumenten steht, ist keine.** Sicherheitsprüfung: Im veröffentlichten instanz-agents-Ordner liegen keine echten Zugangsdaten (nur .env.example).
